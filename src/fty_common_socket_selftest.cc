/*  =========================================================================
    fty_common_socket_selftest.c - run selftests
...
#include "fty_common_socket_classes.h"
...
// Tests for stable public classes:
    { "fty_common_socket_sync_client", fty_common_socket_sync_client_test, true, true, NULL },
    { "fty_common_socket_basic_mailbox_server", fty_common_socket_basic_mailbox_server_test, true, true, NULL },
...
{
    test_item_t *item;
    printf ("Running fty-common-socket selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_COMMON_SOCKET_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_common_socket_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_common_socket_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-common-socket test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_COMMON_SOCKET_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_common_socket_private_selftest (verbose, test->subtest);
#endif // FTY_COMMON_SOCKET_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
